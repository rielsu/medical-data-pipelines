...
for input_path, output_path in zip(input_paths, output_paths):
    ...
    timestamp_format_date = "yyyy-MM-dd"

    if "date" in df.columns:
        df = df.withColumn(
            "date",
            coalesce(
                to_timestamp(df["date"], timestamp_format),
                to_timestamp(df["date"], timestamp_format_date),
            ),
        )

    if "expiration_date" in df.columns:
        df = df.withColumn(
            "expiration_date", to_timestamp(df["expiration_date"], timestamp_format)
        )
